...
6. After the code block write ONE sentence describing what you built
...
export default function App() {
  ...
  return (
    <div style={{height:"100vh",background:C.bg,fontFamily:"monospace",color:C.text,display:"flex",flexDirection:"column",overflow:"hidden"}}>
      ...
      {/* PREVIEW — TOP 55% */}
      <div style={{height:"55%",display:"flex",flexDirection:"column",borderBottom:`1px solid ${C.border}`,flexShrink:0}}>
      ...
      <div style={{flex:1,display:"flex",flexDirection:"column",overflow:"hidden",minHeight:0}}>
        ...
        <div style={{padding:"8px 12px",borderBottom:`1px solid ${C.border}`,background:C.sidebar,display:"flex",flexWrap:"wrap",gap:4,flexShrink:0}}>
          {QUICK.map(p=>(
            <button key={p} onClick={()=>{setInput(`Build a ${p}`);setTimeout(()=>inputRef.current?.focus(),50);}}
              style={{background:"transparent",border:`1px solid ${C.border}`,color:C.muted,padding:"3px 10px",fontFamily:"monospace",fontSize:10,cursor:"pointer",borderRadius:3}}>
              {p}
            </button>
          ))}
        </div>
        ...
        {/* Messages */}
        <div style={{flex:1,overflowY:"auto",padding:"10px 12px",display:"flex",flexDirection:"column",gap:8,background:C.chatBg,minHeight:0}}>
          {msgs.map((msg,i)=>(
            <div key={i} style={{display:"flex",flexDirection:"column",alignItems:msg.role==="user"?"flex-end":"flex-start"}}>
              <div style={{maxWidth:"80%",background:msg.role==="user"?C.accent:C.panel,border:`1px solid ${msg.role==="user"?C.accent:C.border}`,padding:"8px 12px",fontSize:13,lineHeight:1.55,color:msg.role==="user"?"#fff":C.text,borderRadius:6}}>
                <span style={{whiteSpace:"pre-wrap"}}>
                  {msg.content.replace(/```[\s\S]*?```/g,"").replace(/\*\*(.*?)\*\*/g,"$1").trim()}
                </span>
                {msg.hasCode&&(
                  <div style={{marginTop:6,padding:"4px 8px",background:"#0a1a0a",border:"1px solid #1a3a1a",color:C.green,fontSize:11,borderRadius:3}}>
                    ✓ Preview updated — look above!
                  </div>
                )}
              </div>
            </div>
        ...
        <div style={{padding:"8px 12px 10px",borderTop:`1px solid ${C.border}`,background:C.sidebar,flexShrink:0}}>
          <div style={{display:"flex",gap:8,alignItems:"flex-end"}}>
            <div style={{flex:1,background:C.inputBg,border:`2px solid ${input.trim()?C.accent:C.border}`,borderRadius:6,transition:"border-color 0.2s"}}>
              <textarea
                ref={inputRef}
                value={input}
            ...
            <button
              ...
              style={{
                background:input.trim()&&!loading?C.accent:"#1a1a28",
