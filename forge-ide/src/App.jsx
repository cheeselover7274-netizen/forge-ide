...
CRITICAL RULES:
1. ALWAYS output a complete HTML file with ALL code inside it
2. NEVER use placeholders or "add your code here"
3. Games must be fully playable - include all game logic
4. Use beautiful dark themes with smooth animations
5. After the code block write ONE sentence about what you built
...
For games: include canvas or DOM-based gameplay, controls, scoring, game loop - everything
For apps: include all functionality, data, interactions
For EDIT requests: output the FULL updated file with the change applied`;
...
<h1>Forge IDE</h1>
<p>Your AI coding partner is ready.<br/>Type what you want in the chat →</p>
...
const QUICK = ["Snake game","Tetris","Todo app","Calculator","Landing page","Dashboard","Pomodoro timer","Kanban board","Tic tac toe","Color picker"];
const AQUICK = ["Review my code for bugs","Suggest new features","Add a dark mode","How can I improve this?"];
...
export default function App() {
  ...
  const send = async () => {
    ...
    const apiMsgs = [
      ...
      {role:"user",content:`${p}\n\nCurrent code (first 400 chars): ${previewHtml.slice(0,400)}`}
    ];
    ...
    <div style={{minHeight:"100vh",background:C.bg,display:"flex",alignItems:"center",justifyContent:"center",fontFamily:"monospace",padding:16}}>
      <div style={{width:"100%",maxWidth:380}}>
        ...
        <div style={{background:C.panel,border:`1px solid ${C.border}`,padding:28,borderRadius:6}}>
          ...
          <button onClick={login} style={{width:"100%",background:C.accent,border:"none",color:"#fff",padding:14,fontFamily:"monospace",fontSize:15,fontWeight:900,cursor:"pointer",letterSpacing:2,borderRadius:3}}>
            LOGIN →
          </button>
        ...
        <div style={{flex:1,overflow:"hidden",display:"flex",flexDirection:"column"}}>
            ...
            <div style={{flex:1,overflowY:"auto",padding:"24px 28px"}}>
              ...
              <div style={{background:C.panel,border:`1px solid ${C.border}`,padding:22,borderRadius:6}}>
                ...
                {history.slice(0,8).map(h=>(
                  <div key={h.id} style={{padding:"10px 0",borderBottom:`1px solid ${C.border}`,fontSize:13,display:"flex",justifyContent:"space-between"}}>
            ...
            <div style={{flex:1,display:"flex",flexDirection:"column",overflow:"hidden",background:"#080e08"}}>
              <div style={{background:"#080e08",borderBottom:"1px solid #1a2a1a",padding:"14px 22px",display:"flex",alignItems:"center",gap:12,flexShrink:0}}>
                ...
                <div>
                  ...
                  <div style={{fontSize:10,color:"#2a5a2a",letterSpacing:1,marginTop:2}}>PRIVATE · ADMIN ONLY</div>
                </div>
              </div>
                ...
                <div style={{display:"flex",flexWrap:"wrap",gap:5,marginBottom:8}}>
                  {AQUICK.map(p=>(
                    <button key={p} onClick={()=>{setAdminInput(p);adminRef.current?.focus();}}
                      style={{background:"transparent",border:"1px solid #1a3a1a",color:"#2a5a2a",padding:"3px 10px",fontFamily:"monospace",fontSize:9,cursor:"pointer",borderRadius:3}}>{p}</button>
                  ))}
                </div>
              </div>
          ...
          {adminTab==="history"&&(
            <div style={{flex:1,overflowY:"auto",padding:"24px 28px"}}>
              ...
              {history.length===0&&<div style={{color:C.muted}}>No builds yet!</div>}
              {history.map(h=>(
                <div key={h.id} style={{background:C.panel,border:`1px solid ${C.border}`,padding:"14px 18px",marginBottom:8,borderRadius:6,display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                  <span style={{color:C.accent,flex:1,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{h.prompt}</span>
  ...
  return (
    <div style={{height:"100vh",background:C.bg,fontFamily:"monospace",color:C.text,display:"flex",flexDirection:"column",overflow:"hidden"}}>
      ...
      <div style={{flex:1,display:"flex",overflow:"hidden",minHeight:0}}>
        {/* LIVE PREVIEW - takes full left side */}
        <div style={{flex:1,display:"flex",flexDirection:"column",overflow:"hidden",minHeight:0}}>
          <div style={{height:36,background:C.panel,borderBottom:`1px solid ${C.border}`,display:"flex",alignItems:"center",padding:"0 16px",gap:8,flexShrink:0}}>
            ...
            {history.length>0&&<span style={{marginLeft:"auto",fontSize:10,color:"#2a3a4a"}}>Built: {history[0]?.prompt?.slice(0,30)}{history[0]?.prompt?.length>30?"...":""}</span>}
          </div>
          <iframe
            key={previewKey}
            srcDoc={previewHtml}
                ...
                <div style={{maxWidth:"92%",background:msg.role==="user"?C.accent:C.panel,border:`1px solid ${msg.role==="user"?C.accent:C.border}`,padding:"10px 13px",fontSize:13,lineHeight:1.6,color:msg.role==="user"?"#fff":C.text,borderRadius:6}}>
                  ...
                  {msg.hasCode&&(
                    <div style={{marginTop:8,padding:"6px 10px",background:"#0a1a0a",border:"1px solid #1a3a1a",color:C.green,fontSize:11,borderRadius:3,display:"flex",alignItems:"center",gap:6}}>
                      <span>✓</span> Preview updated — look left!
                    </div>
                  )}
                </div>
              </div>
            ...
            {loading&&(
              <div style={{alignSelf:"flex-start"}}>
                <div style={{background:C.panel,border:`1px solid ${C.border}`,padding:"12px 16px",display:"flex",gap:5,alignItems:"center",borderRadius:6}}>
                  ...
                  <span style={{fontSize:11,color:C.muted,marginLeft:6}}>Building your app...</span>
                </div>
              </div>
            )}
            <div ref={chatEnd}/>
